...
//
//
//bool table_original_method(unsigned plain_num, const vector<text>& pp) {  //新方法,明文个数为plain_num，明文空间为pp
//	map<text, text>  table;  //表里为（p^f^-1(c)，c）
//	text k_1(0, 0);
//
//	for (int i = 0; i != plain_num; i++) {  //对每个已知明文求p^f^-1(c)，写到表中，找碰撞
//		text p = pp[i];
//		text c = em(p);
//		text f_p = permu32(p);  //f_p=f(p)
//		text firstcord = c^f_p;  //表中的首坐标
//
//
...
//}
//
//bool table_new_method(unsigned plain_num, const vector<text>& pp) {  //新方法,明文个数为plain_num，明文空间为pp
//	map<text, text>  table;  //表里为（p^f^-1(c)，c）
//	text k_2(0, 0);
//
//	for (int i = 0; i != plain_num; i++) {  //对每个已知明文求p^f^-1(c)，写到表中，找碰撞
...
//		srand(unsigned(time(0)));
//
//		text bound(bound1, 0);  //上界(从65536开始)
//
...
//
//			unsigned int k_2 = 0;
//			unsigned int  plain = 65536;  //设置给定明文个数
//
