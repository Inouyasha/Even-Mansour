...
bool table_original_method(unsigned plain_num, const vector<text>& pp, const vector<text>& delta) {  //新方法,明文个数为plain_num，明文空间为pp	
	map<text, text>  table;  //表里为E(P)\oplus P==>p
	...
	for (int i = 0;i != delta.size();i++) {
		text x = delta[i];
		text pivot = permu32(x) ^ x;  //F(x)\oplus x
...
bool table_new_method(unsigned plain_num, const vector<text>& pp, const vector<text>& delta) {  //新方法,明文个数为plain_num，明文空间为pp
	...
	for (int i = 0;i != delta.size();i++) {
		text x = delta[i];
		text pivot = permu32_re(x) ^ x;  //F(x)\oplus x
		if (table.find(pivot) != table.end()) {
			k_1 = table[pivot] ^ x;
			if (k_1 == k1)
				return true;
...
int main() {
	for (int bound1 = 1;bound1 < 65536;bound1*=4) {
		srand(unsigned(time(0)));

		text bound(bound1, 0);  //上界(从16384开始)
		...
		for (int i = 0;i != count;i++) {  //500测试，用来计算随机实例的成功性
			...
			unsigned int k_2 = 0;
			unsigned int  plain = 66536;  //设置给定明文个数
			

			vector<text> pp(plain, text(0, 0));
			permu_plain(pp, bound);  //生成给定明文

			unsigned delta_num = 65536;
